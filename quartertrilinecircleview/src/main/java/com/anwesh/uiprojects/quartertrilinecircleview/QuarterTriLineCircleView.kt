...
class QuarterTriLineCircleView(ctx : Context) : View(ctx) {
    ...
    data class QTLCNode(var i : Int, val state : State = State()) {

        private var next : QTLCNode? = null
        private var prev : QTLCNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawQTLCNode(i, state.scale, paint)
        ...
        fun getNext(dir : Int, cb : () -> Unit) : QTLCNode {
            var curr : QTLCNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class QuarterTriLineCircle(var i : Int) {

        private val root : QTLCNode = QTLCNode(0)
        private var curr : QTLCNode = root
